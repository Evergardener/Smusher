...
if(!defined("BLUDIT")) { die("You seek for fresh air and peace of mind, Ivanov? I'm in the North, waiting for you."); }
			$smush_params = array('quality', 'exif');
			if(@$_GET['disable'] == 1 && @$_GET['config']==1){
				copy(PATH_AJAX.'upload-images.bak.php', PATH_AJAX.'upload-images.php');
				$config['disable'] = 1;
				$config['enable'] = 0;
				file_put_contents($settings_file, json_encode($config));
				echo '<h1>Plugin disabled</h1>'.'<meta http-equiv="refresh" content="1;URL=' . $site->url() . ADMIN_URI_FILTER. '/smusher' . '">';
				exit(0);
			}
			if(@$_GET['enable'] == 1 && @$_GET['config']==1){
				copy($PATH_ROOT. 'core' . DS .'upload-images.php', PATH_AJAX.'upload-images.php');
				$config['disable'] = 0;
				$config['enable'] = 1;
				file_put_contents($settings_file, json_encode($config));
				echo '<h1>Plugin enabled</h1>'.'<meta http-equiv="refresh" content="1;URL=' . $site->url() . ADMIN_URI_FILTER. '/smusher' . '">';
				exit(0);
			}
			...
			if($_GET['quality'] > 100 OR $_GET['quality'] < 5){
				die("Quality setting is wrong, do not play with me, I'm young an inexperienced, I'll get broken.");
			}
			$config['quality'] = (int)$_GET['quality'];
			$config['filemask'] = $_GET['filemask'];
			if(@$_GET['exif'] == 'on'){
				$config['exif'] = 1;
			}
			else{	$config['exif'] = 0;	}
			file_put_contents($settings_file, json_encode($config));
			echo '<h1>Settings saved</h1>'.'<meta http-equiv="refresh" content="1;URL=' . $site->url() .  ADMIN_URI_FILTER . '/smusher' . '">';
			exit(0);
			}
			$quality = $config['quality'];
			$exif = $config['exif'];
			$saved = round((int)$config['saved'] / 1024 / 1024, 3);
			$url = $site->url() . ADMIN_URI_FILTER."/smusher";
			switch($exif){
				case 1:
				$exif = 'checked';
				break;
			...
			$url_back = $site->url() . ADMIN_URI_FILTER . '/plugins'; 
			if((int)$config['disable'] == 1){
				$button_url = $site->url() . ADMIN_URI_FILTER . "/smusher?config=1&enable=1";
				$text_button = 'Enable Smusher';
			}
			else{
				$button_url = $site->url() . ADMIN_URI_FILTER . "/smusher?config=1&disable=1";;
				$text_button = 'Disable Smusher';
			}
			//Little playground
			$html = 
			
<<<EOF
<br><input type="button" style="margin-left: 2px;" class="btn btn-primary btn-sm" value="Go back" onClick="document.location = '$url_back'"/>
<input type="button" style="margin-right: 2px;" class="btn btn-primary btn-sm" value="$text_button" onClick="document.location = '$button_url'"/> 
<br><br><center><img src="$current_html_directory/core/banner.png" weight="100%" height="260"></center>
<br><center><form action="$url" method="GET">
Quality: <input type="number" name="quality" value="$quality" min="5" max="100" /><br><br>
<input type="hidden" value="1" name="config"/>
Exif: <input type="checkbox" name="exif" $exif /><br><br>
File mask: <input type="text" name="filemask" value="$filemask" /><br><br>
<span class="tip">*Exif is additional information in the file content, such as the camera model, date, GPS coordinates of the location where the shooting took place, the author's name, website, and so on...</span><br><br>
<input type="submit" value="Save"></center><br><p style="text-align: right; margin-right:10px;">Overall memory saved: <span style="color: #049500;">$saved MB</span></p>
...
input[type=submit] {
    ...
    font-family: Tahoma;
    background: #dbdbdb;
    background: linear-gradient(to bottom, #A42 1%, #049500 100%);
}
html, body {
...
document.title = "Smusher Configurator";
'use strict';
...
EOF;
			if(BLUDIT_VERSION<3){
				$html = '<h2 style="color:red;text-align:center;">Your Bludit version is too old for Smusher<br> Minimal version is 3.13.0</h2>';
			}
			if(!isset($config['quality'])){
				$html = "Something went wrong, did you change plugin's directory permissions recursively to read/write(777) ?";
			}
			echo $html;
			exit(0);
